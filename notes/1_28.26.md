...
- Python equivalent for gdal is `rasterio` library (does like all of the same operations)
- Python for projects is `pyproj` library
...
## QGIS
- Map type in bottom right corner

## Current satellites
- **NASA**: Landsat
    - Revisit time is like 10 days
    - Iterations from 1-9 from like 1980s-present
    - Each iteration senses different optical bands
    - 30 meter resolution, 100 meter thermal resolution
- **ESA**: Sentinel-2
    - Revisit time is like 5 days (as there is 2/3 satellites in different orbits)
    - 10 meter resolution
- Suath width = width that gets scanned while imaging over a region
- Use sun-synchronous orbit so that local-times are all aligned and the time-series analysis works
- Near-infrared is useful for vegetation mapping as it's very reflective in the NIR bands


## Tradeoffs
- Resolution tradeoff in satellite missions
- 4 main axes:
    1. Spatial resolution
    2. Spectral resolution
    3. Temporal resolution
    4. Radiometric resolution
- Sentinel 2 and Landsat all balance spatial/spectral/temporal really well
    - 10m resolution, 10 bands imaged, 5 day revist
    - Companies like Planet are optimizing for the spatial part
